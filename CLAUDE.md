...
- **Session 8 (2026-05-18)** — Twilio SMS unblocked. Corrected MACO Account SID in Vercel (the SID in env since 2026-03-25 was wrong — the correct one is stored in memory `twilio-account-sid-was-wrong`). SMS consent dropped from "required to submit" to "optional" on `/apply` and `/apply-oo` to satisfy CTIA rule 5.1.1 — fix was 6 weeks late. Webhook honors `sms_consent: false` by skipping welcome SMS + drip enrollment for that lead.
- **Session 9 (2026-05-20)** — Conversion-failure deep dive after Jacob confirmed zero phone calls from FB campaigns. 48h data: 510 page_views → 4 form_starts (0.78%) → 0 submits → 0 new leads. Meta side verified healthy (campaigns active, no rejections, account good, balance solid). Diagnosed root cause as Facebook in-app browser (FBIOS / FB4A — 100% of FB traffic) breaking interaction with the React form, compounded by Advantage Audience routing spend to non-CDL drivers. Actions: (1) mobile-first redesign of `/apply` + `/apply-oo` with form above the fold and 3-field quick-apply (rest in `<details>` expander); (2) "FB in-app browser" detection banner with prominent tap-to-call CTA + "Open in Safari/Chrome" instructions; (3) `low_form_engagement` watcher added to meta-optimize cron (PV→start < 5% / start→submit < 25% / PV→submit < 1% thresholds); (4) **paused Company Drivers - Traffic - Apr2026** + **reduced Owner Operators - Memphis - Apr2026 daily budget to $5** via Meta API (was $30 each, saving $55/day); (5) restricted Owner Ops placements to Facebook feed + Instagram feed only (eliminated Audience Network / Marketplace / Reels / Stories); could NOT disable Advantage Audience due to Meta API constraint with age_range — see memory `meta-advantage-audience-locked`. (6) Removed "new 2024 Freightliners" / "new equipment" from all copy after user flagged inaccuracy. (7) Fixed `ai_campaigns` schema mismatch (same class of bug as `cmo_inbox_items` in Session 7 — real columns are `campaign_name` / `ad_copy_headline` / `ad_copy_body` / `cta_text` / `target_audience`). (8) Staged 4 phone-first ad copy variants in `ai_campaigns` (Phone-First Urgent, Pay-Driven $80K, Anti-OTR, Referral $500 Bonus) — all draft status, visible at `/dashboard/ai-cmo/ads`. (9) Drafted (not deployed) referral activation drip targeting hired drivers at day 30/60/90.
